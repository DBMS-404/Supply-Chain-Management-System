...
--
-- Host: 127.0.0.1
-- Generation Time: Jun 27, 2022 at 06:15 AM
...
--
-- Stand-in structure for view `available_trucks`
-- (See below for the actual view)
--
...
,`cons_turn_count` int(11)
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `employee_full_details`
-- (See below for the actual view)
--
CREATE TABLE `employee_full_details` (
`user_id` varchar(20)
,`mobile_no` varchar(20)
,`weekly_worked_hours` int(11)
,`city` varchar(50)
...
INSERT INTO `item_assignment` (`item_id`, `order_id`) VALUES
(1, 1),
(2, 3),
(3, 2),
(4, 2),
...
(8, 'CR2', 2, '2022-06-25', 'dtrain', 'Katubedda, Moratuwa', 30);
...
(112344, 'Udarata Manike', '2022-06-16', '03:58:03', 'Matara', 10000, 8788, 0),
(1123245, 'Yal devii', '2022-06-16', '03:58:03', 'Jaffna', 10000, 3358, 0);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`sql6501921`@`%` SQL SECURITY INVOKER VIEW `available_drivers`  AS SELECT `driver`.`user_id` AS `user_id`, `user`.`first_name` AS `first_name`, `user`.`last_name` AS `last_name`, `employee`.`mobile_no` AS `mobile_no`, `employee`.`weekly_worked_hours` AS `weekly_worked_hours`, `employee`.`city` AS `city` FROM ((`driver` join `employee` on(`driver`.`user_id` = `employee`.`user_id`)) join `user` on(`driver`.`user_id` = `user`.`user_id`)) WHERE !(`driver`.`user_id` in (select `leaved_employees_today`.`user_id` from `leaved_employees_today`)) AND hour(timediff(cast(convert_tz(current_timestamp(),'+00:00','+05:30') as time),`employee`.`last_arrival_time`)) > 0 ;

-- --------------------------------------------------------

--
-- Structure for view `available_trucks`
--
DROP TABLE IF EXISTS `available_trucks`;

CREATE ALGORITHM=UNDEFINED DEFINER=`sql6501921`@`%` SQL SECURITY INVOKER VIEW `available_trucks`  AS SELECT `truck`.`truck_id` AS `truck_id`, `truck`.`truck_no` AS `truck_no`, `truck`.`city_id` AS `city_id` FROM (`truck` left join `turn` on(`truck`.`truck_id` = `turn`.`truck_id`)) WHERE `turn`.`scheduled_date` is null OR `turn`.`scheduled_date` = cast(convert_tz(current_timestamp(),'+00:00','+05:30') as date) AND (`turn`.`turn_start_time` is null OR `turn`.`turn_end_time` is not null) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`sql6501921`@`%` SQL SECURITY INVOKER VIEW `employee_details`  AS SELECT `employee`.`user_id` AS `user_id`, `employee`.`mobile_no` AS `mobile_no`, `employee`.`weekly_worked_hours` AS `weekly_worked_hours`, `employee`.`city` AS `city`, `employee`.`last_arrival_time` AS `last_arrival_time`, `driver_assistant`.`cons_turn_count` AS `cons_turn_count` FROM (`employee` left join `driver_assistant` on(`employee`.`user_id` = `driver_assistant`.`user_id`)) ;

-- --------------------------------------------------------

--
-- Structure for view `employee_full_details`
--
DROP TABLE IF EXISTS `employee_full_details`;

CREATE ALGORITHM=UNDEFINED DEFINER=`sql6501921`@`%` SQL SECURITY INVOKER VIEW `employee_full_details`  AS SELECT `employee`.`user_id` AS `user_id`, `employee`.`mobile_no` AS `mobile_no`, `employee`.`weekly_worked_hours` AS `weekly_worked_hours`, `employee`.`city` AS `city`, `employee`.`last_arrival_time` AS `last_arrival_time`, `user`.`first_name` AS `first_name`, `user`.`last_name` AS `last_name`, `city`.`name` AS `city_name`, `driver_assistant`.`cons_turn_count` AS `cons_turn_count` FROM (((`employee` join `city` on(`city`.`city_id` = `employee`.`city`)) left join `user` on(`employee`.`user_id` = `user`.`user_id`)) left join `driver_assistant` on(`employee`.`user_id` = `driver_assistant`.`user_id`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`sql6501921`@`%` SQL SECURITY INVOKER VIEW `leaved_employees_today`  AS SELECT DISTINCT `employee_leave`.`user_id` AS `user_id` FROM `employee_leave` WHERE `employee_leave`.`date` = cast(convert_tz(current_timestamp(),'+00:00','+05:30') as date) AND `employee_leave`.`status` = 1 ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`sql6501921`@`%` SQL SECURITY INVOKER VIEW `order_details`  AS SELECT `item_order`.`order_id` AS `order_id`, `item_order`.`user_id` AS `user_id`, `item_order`.`route_id` AS `route_id`, `item_order`.`date` AS `date`, `item_order`.`status` AS `status`, `item_order`.`address` AS `address`, `item_order`.`weight` AS `weight`, `city_assignment`.`city_id` AS `city_id` FROM (`item_order` join `city_assignment` on(`item_order`.`route_id` = `city_assignment`.`route_id`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`sql6501921`@`%` SQL SECURITY INVOKER VIEW `turns_to_dispatch`  AS SELECT `turn`.`turn_id` AS `turn_id`, `turn`.`driver_id` AS `driver_id`, `turn`.`assistant_id` AS `assistant_id`, `turn`.`truck_id` AS `truck_id`, `turn`.`route_id` AS `route_id`, `city_assignment`.`city_id` AS `city_id` FROM (`turn` join `city_assignment` on(`turn`.`route_id` = `city_assignment`.`route_id`)) WHERE `turn`.`turn_start_time` is null ;
